...
#Para isso, é necessario tratar a coluna DTNASC para descobrir qual dia da semana
#está associado aquela data. 

#Conforme é observado falta-se um 0 para esquerda para todas as datas. Portanto:
...
#Primeiramente atribuindo uma coluna com o numero total de nascimento para
...
#Diante a independencia desses eventos, calcua-se a probabilidade conjunta
...
#Diante a independencia desses eventos, calcua-se a probabilidade conjunta do parto e turno
...
#Para descobrir qual a probabilidade de ocorrência de x nascimentos por dia,
#necessita utilizar o conceito de um modelo de distribuição de probabilidade discreta
#Atentando que ao tamanho da base de dados, a distribuição mais adequada é a 
#distribuição de Poisson. 

#Para descobrir a taxa média de ocorrência do evento em uma unidade medida,
#necessita-se descobrir a médio de nascimento a depender do tipo de parto e turno
#Observação: o ano de 2016 teve 366 dias 

#Com o código abaixo, é respondido a pergunta:
#Por dia, quantos nascimentos ocorrem em média no turno da manhã por parto cesário?

base %>%
  group_by(turno, PARTO) %>% 
  summarise(media = n()/366)
  
#RESPOSTA: em média no turno da manhã nascem 2,43 crianças por parto cesário. 


#Obtendo essas informações das médias e atribuindo a variavel aleatoria discreta como
#o número de ocorrência de x nascimentos/dia, podemos calcular a probabilidade de 
#ter zero, um, dois e assim por diante nascimentos por dia . 

#Abaixo segue o cálculo da  distribuição de Poisson, considerando que o valor médio do turno do 
#dia por parto cesário é igual 2.43:
...
#Considerando o valor médio do turno do dia para parto normal (2.40)
poisson = function(x, lambda){
  p = exp(-lambda)*lambda^x/factorial(x)
  return(p)
}
...
#Mostrando graficamente a distribuição de probabilidade tendo em vista até 5 
#nascimentos por tipo de parto no turno Dia 

#Construbindo uma tabela com os dados das probabilidades de parto cesáreo
x <- c(0,1,2,3,4,5)
...
####################################################################
#Agora considerando o turno da noite para parto cesáreo (1.66)
poisson = function(x, lambda){
  p = exp(-lambda)*lambda^x/factorial(x)
  return(p)
}
...
#Considerando o valor médio do turno da noite para parto normal (2.20)
poisson = function(x, lambda){
  p = exp(-lambda)*lambda^x/factorial(x)
  return(p)
}
...
#Mostrando graficamente a distribuição de probabilidade tendo em vista até 5 
#nascimentos por tipo de parto no turno Noite 
...
#Descobrindo a taxa de ocorrência desses eventos 

base %>%
  group_by(PARTO, dia_semana) %>% 
  summarise(media = n()/366)

#Domingo tem uma taxa média de parto cesário igual a 0,628. Portanto
#a função de probabilidade é igual a: 
...
#Fazendo a analise para parto normal de acordo com os dias, respectivamente são:
